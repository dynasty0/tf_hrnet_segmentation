...
from net.head import ClsHead, SegHead
...
class HRNet():
    ...
    def calc_loss(self, labels, outputs, trainable_vars):
        ...
        # return loss, metrics
            ...
            size = labels.get_shape()[1:3]
            outputs = tf.image.resize_images(outputs, size)
            loss = tf.reduce_mean(tf.losses.sparse_softmax_cross_entropy(tf.squeeze(tf.to_int32(label),3), outputs))
            l2_loss = tf.add_n([tf.nn.l2_loss(var) * self.cfg['NET']['weight_l2_scale'] for var in trainable_vars ])
            loss += l2_loss
    ...
    def _build_components(self):
        ...
        if self.cfg['MODEL']['type'] == 0: ###分类
            ...
            seghead = SegHead(self.cfg)
            self.stages.append(seghead)
